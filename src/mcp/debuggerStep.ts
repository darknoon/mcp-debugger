...
server.tool(
  "debuggerStep",
  `Makes a single step forward while the debugger is paused (because of a breakpoint perhaps)`,
  {
    sessionId: z
      .string()
      .optional()
    ...
    threadId: z
      ...
      .optional()
      .describe(
        "The thread ID to step (uses last stopped thread if not provided)",
      ),
  },
  async ({ sessionId, threadId }) => {
    ...
    const response = await session.request("next", {
