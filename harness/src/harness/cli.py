...
@click.option(
    "--temp-dir", "-t", type=click.Path(), help="Base directory for temporary files"
)
def fix(instance_id: str, dataset: str, output: Optional[str], temp_dir: Optional[str]):
    ...
    if not example:
        click.echo(
            f"Error: Instance ID '{instance_id}' not found in dataset {dataset}",
            err=True,
        )
        sys.exit(1)
...
@click.option("--limit", "-l", type=int, help="Limit number of examples to process")
def fix_all(
    dataset: str, output_dir: str, temp_dir: Optional[str], limit: Optional[int]
):
    ...
    for i, instance_id in enumerate(instance_ids, 1):
        ...
        try:
            ...
            results.append(
                {"instance_id": instance_id, "success": False, "error": str(e)}
            )
