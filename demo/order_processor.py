...
class OrderProcessor:
    ...
    def _init_demo_data(self):
        ...
        self.products = {
            ...
            "HEADPHONES001": Product(
                "HEADPHONES001", "Wireless Headphones", 199.99, "Electronics"
            ),
            "KEYBOARD001": Product(
                "KEYBOARD001", "Mechanical Keyboard", 149.99, "Electronics"
            ),
        }
        ...
        try:
            ...
            for product_id, allocations in order.warehouse_allocations.items():
                for warehouse_id, quantity in allocations.items():
                    self.warehouses[warehouse_id].commit_reservation(
                        product_id, quantity
                    )
    ...
    def create_order(
        self, customer_id: str, products: List[Tuple[str, int]], priority: int = 0
    ) -> Order:
        with self.processing_lock:
...
def simulate_concurrent_orders():
    ...
    # Create customers
    customers = [
        processor.create_customer(f"CUST{i:03d}", f"Customer {i}") for i in range(5)
    ]
    ...
    for i in range(10):
        ...
        order = processor.create_order(
            customer.id, products, priority=random.randint(0, 2)
        )
    ...
    for customer in customers:
        ...
        for order in customer.order_history:
            if order.discount_applied > 0:
                actual_discount_rate = order.discount_applied / (
                ...
                if abs(actual_discount_rate - expected_discount) > 0.01:
                    ...
                    print(
                        f"  Expected: {expected_discount:.2%}, Got: {actual_discount_rate:.2%}"
                    )
